...
[ $# -ne 3 ] && { echo "Usage: $0 [runs] [cores] [river-libfuzzer-path]"; exit 1; }
read runs cores rlf_path <<<$@
...
CORPUS_DIR=$(readlink -f EXT)
NO_TESTCASES=$(ls -al $CORPUS_DIR | wc -l )
...
while true; do
  left=$(mongo --eval "db.test.find({state : \"new\"}).count()" | tail -n1)
  sleep 5 #todo fix this
  if [ "$left" = "0" ]; then
    break
  fi
done
...
last=$(mongo --eval "db.test.find({}, {tracedTS:1, _id:0}).sort({tracedTS: -1}).limit(1)" | tail -n 1 | grep --only-matching --perl-regex "(?<=\"tracedTS\" : )[^ ]*")

echo "RIVER statistics :"
echo "number_of_testcases : $NO_TESTCASES"
echo "number_of_cores : $cores"
echo "time_in_ms : $((last - fst))"
